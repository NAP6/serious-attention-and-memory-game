...
-- -----------------------------------------------------
-- procedure count_patients_by_group
-- -----------------------------------------------------

DELIMITER $$
USE `serious_game`$$
CREATE PROCEDURE `count_patients_by_group`(IN _admin_id INT)
BEGIN
SELECT 
    JSON_ARRAYAGG(t.count) AS `count`
FROM
    (SELECT 
        JSON_OBJECT(gr.gr_name, COUNT(pa_gr.pa_id)) AS `count`
    FROM
        tb_group gr
    INNER JOIN tb_administrator_has_group ad_gr ON gr.gr_id = ad_gr.gr_id
    LEFT JOIN tb_patient_has_group pa_gr ON gr.gr_id = pa_gr.gr_id
    WHERE
    gr.gr_is_deleted = false and
        ad_gr.adm_id = _admin_id
    GROUP BY gr.gr_name) t;
END$$
