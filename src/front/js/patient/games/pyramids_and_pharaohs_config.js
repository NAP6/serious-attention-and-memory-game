...
function next_level() {
  if (index_level + 1 >= pap.levels.length) {
    ...
    Swal.fire(
      ...
      if (index_level == 0) {
        this_match.start();
        eye_tracking_interval_id = setInterval(
          record_eye_tracking,
          interval_milisecons_to_capture_eye_position
        );
      } else this_match.finish_level(index_level - 1);
...
function finish_game() {
  ...
  Swal.fire(
    ...
    Swal.fire(
      `Gracias`,
      "Los datos se enviaron de forma exitosa",
      "success"
    ).then(() => {
...
async function correct_point(
  ...
  record_event(point, element, true, index_point, level_index, group_name);
  element.classList.add("sucssess");
}

async function error_point(
  point,
  element,
  index_point,
  level_index,
  ...
  record_event(point, element, false, index_point, level_index, group_name);
  var time = 100;
...
function charge_level(level, level_index) {
  if (level) {
    number_of_correct_answers = level.pp_answer.filter((img) => {
      return img.selected;
    }).length;
...
function chage_image(image = "", level_index, is_example) {
  var image_item = document
    .getElementById("album_image_template")
